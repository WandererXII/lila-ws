...
final class History[K <: StringValue, V <: ipc.ClientIn.HasVersion](
  ...
  def version(key: K): Option[(String, SocketVersion)] =
    Option(histories.get(key.toString)).flatMap(_.headOption.map(e => key.toString -> e.version))
...
object History {
  ...
  val room  = new History[RoomId, ipc.ClientIn.Versioned](20, 512)
  val round = new History[Game.Id, ipc.ClientIn.RoundVersioned](20, 2048)
}
